...
import { useTourly } from "./TourlyContext";
import { motion, useInView } from "framer-motion";
import { useRouter } from "next/navigation";
import { Portal } from "@radix-ui/react-portal";

// Types
import { TourlyProps } from "./types";

const Tourly: React.FC<TourlyProps> = ({
  children,
  interact = false,
  steps,
  shadowRgb = "0, 0, 0",
  ...
  const { currentTour, currentStep, setCurrentStep, isTourlyVisible } =
    useTourly();
  ...
  useEffect(() => {
    if (isTourlyVisible && currentTourSteps) {
  ...
  }, [currentStep, currentTourSteps, isInView, offset, isTourlyVisible, interact]);
  ...
  useEffect(() => {
    if (isTourlyVisible && currentTourSteps) {
      console.log("Tourly: Current Step Changed");
      const step = currentTourSteps[currentStep];
      if (step) {
        const element = document.querySelector(step.selector) as Element | null;
        if (element) {
  ...
  }, [currentStep, currentTourSteps, isInView, offset, isTourlyVisible]);

  useEffect(() => {
    if (elementToScroll && !isInView && isTourlyVisible) {
      console.log("Tourly: Element to Scroll Changed");
  ...
  }, [elementToScroll, isInView, isTourlyVisible]);
  ...
  }, [currentStep, currentTourSteps, isTourlyVisible]);
  ...
  return (
    <div
      data-name="tourly-wrapper"
      className="relative w-full"
      data-tourly="dev"
    >
      {/* Container for the Website content */}
      <div data-name="tourly-site" className="block w-full">
        {children}
      </div>

      {/* Tourly Overlay Step Content */}
      {pointerPosition && isTourlyVisible && CardComponent && (
        <Portal>
          {!interact && (
            <div className="fixed inset-0 z-[900]" />
          )}
          <motion.div
            data-name="tourly-overlay"
            className="absolute inset-0 "
            initial="hidden"
            animate={isTourlyVisible ? "visible" : "hidden"}
            variants={variants}
            transition={{ duration: 0.5 }}
          >
            <motion.div
              data-name="tourly-pointer"
